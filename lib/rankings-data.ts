...
function playersStatsSql() {
  return `
    ...
    WHERE COALESCE(u.is_temporary, 0) = 0
  `;
}
...
export async function getRankablePlayers(db: AppDb, seasonId: number): Promise<RankablePlayer[]> {
  ...
  const rows = (await db.prepare(playersStatsSql()).all(seasonId, seasonId)) as PlayerStatsRow[];
...
export async function getTopOverallRankedPlayersForSeason(
  ...
  const players = await getRankablePlayers(db, seasonId);
...
export async function getHomeTopPlayers(limit = 3): Promise<HomeTopPlayer[]> {
  const top = await getTopOverallRankedPlayers(limit);
  return top.map((p) => ({
    rank: p.rank,
    userId: p.userId,
...
export async function getRankingsPageData(requestedSeasonId?: number | null) {
  const db = await getDb();
  const { season, seasons } = await resolveRankingSeasonForView(db, requestedSeasonId);
  if (!season) {
    return { season: null, seasons, players: [] as RankablePlayer[] };
  }
  const players = await getRankablePlayers(db, season.id);
