...
/** Usuwa gościa jednorazowego po potwierdzeniu płatności za mecz. */
export async function removeTemporaryGuestIfPaid(args: {
  userId: number;
  matchId: number;
  actorUserId?: number;
}): Promise<boolean> {
  ...
  await db.prepare("DELETE FROM match_wallet_charges WHERE user_id = ?").run(uid, mid);
  ...
  if (args.actorUserId != null) {
    await logActivity(
      args.actorUserId,
      `Usunął gościa ${user.first_name} ${user.last_name} po potwierdzeniu płatności (mecz id ${mid})`
    );
  }
