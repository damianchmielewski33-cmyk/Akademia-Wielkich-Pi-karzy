...
function createTestDb(): { db: AppDb; sqlite: Database.Database; dbPath: string } {
  ...
  sqlite.exec(`
    ...
    CREATE TABLE hotpay_payments (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT NOT NULL UNIQUE, user_id INTEGER NOT NULL, kind TEXT NOT NULL, amount_pln REAL NOT NULL, status TEXT NOT NULL DEFAULT 'pending', deposit_request_id INTEGER, created_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY (user_id) REFERENCES users(id));
    ...
    CREATE TABLE ad_impressions (id INTEGER PRIMARY KEY AUTOINCREMENT, slot_id TEXT NOT NULL, screen_key TEXT NOT NULL, pathname TEXT NOT NULL, user_id INTEGER, visitor_id TEXT NOT NULL, created_at TEXT NOT NULL, placement TEXT NOT NULL DEFAULT 'footer', fill_status TEXT NOT NULL DEFAULT 'pending', FOREIGN KEY (user_id) REFERENCES users(id));
    CREATE TABLE cookie_consent_events (id INTEGER PRIMARY KEY AUTOINCREMENT, choice TEXT NOT NULL, visitor_id TEXT NOT NULL, created_at TEXT NOT NULL);
    ...
    CREATE TABLE venue_applications (id INTEGER PRIMARY KEY AUTOINCREMENT, contact_name TEXT, contact_email TEXT, contact_phone TEXT, venue_name TEXT, city TEXT, address TEXT, status TEXT, partner_user_id INTEGER, reviewed_by_admin_id INTEGER, created_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY (partner_user_id) REFERENCES users(id), FOREIGN KEY (reviewed_by_admin_id) REFERENCES users(id));
    CREATE TABLE venue_payouts (id INTEGER PRIMARY KEY AUTOINCREMENT, venue_id INTEGER NOT NULL, gross_pln REAL, platform_fee_pln REAL, owner_payout_pln REAL, booking_count INTEGER, status TEXT, created_by_admin_id INTEGER NOT NULL, paid_by_admin_id INTEGER, created_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY (created_by_admin_id) REFERENCES users(id));
