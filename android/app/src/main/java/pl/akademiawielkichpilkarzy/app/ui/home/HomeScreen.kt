...
fun HomeScreen(
    ...
    // Splash schodzi dopiero po namalowaniu treści (nie w trakcie LoadingBlock).
    LaunchedEffect(loading, error, data) {
        if (loading) return@LaunchedEffect
        if (onInitialContentReady == null) return@LaunchedEffect
        withFrameNanos { }
        withFrameNanos { }
    ...
    val tiles = buildList {
        ...
        add(
            HomeTile(
                ...
                // Custom Tabs — nie WebView AWP (sendBeacon GymBrat potrafił nawigować
                // na POST /api/analytics/page-view i blokować otwarcie strony).
                onClick = {
                    openExternalUri(context, Uri.parse(SisterSites.gymBratCrossLink("/")))
                }
            )
        )
        if (nav.isAdmin) {
