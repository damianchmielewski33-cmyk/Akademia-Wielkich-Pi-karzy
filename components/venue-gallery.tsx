...
export function VenueGallery({ photos, name }: Props) {
  ...
  if (urls.length === 1) {
    return (
      ...
      <img src={current} alt={name} className="h-52 w-full object-cover sm:h-[28rem]" />
    );
  }
      ...
      {urls.slice(1).map((url, index) => (
        <button key={url} type="button" onClick={() => setActive(index + 1)} className="relative hidden sm:block">
          ...
          <img
            src={url}
            alt=""
            className={cn("h-full w-full object-cover", active === index + 1 ? "ring-2 ring-inset ring-white" : "")}
          />
        </button>
      ))}
      <div className="mp-h-scroll gap-2 p-2 sm:hidden">
        {urls.map((url, index) => (
          <button
            key={url}
            type="button"
            ...
            className={cn(
              "h-16 w-24 shrink-0 overflow-hidden rounded-xl",
              index === active ? "ring-2 ring-[var(--mp-teal)]" : ""
            )}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
