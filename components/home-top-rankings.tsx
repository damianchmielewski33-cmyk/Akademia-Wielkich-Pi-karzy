    ...
    <>
      <span
        className={cn(
          ...
          photoSrc
            ? "bg-[var(--mundial-gold,#f5c518)] text-zinc-950"
      ...
      <PlayerAvatar
        ...
        ringClassName={photoSrc ? "ring-2 ring-white/60" : "ring-2 ring-[var(--mp-teal)]/25"}
      />
      <div className="mt-3 w-full min-w-0">
        <PlayerNameStack
          ...
          primaryClassName={
            photoSrc ? "font-bold text-white drop-shadow-sm" : "font-bold text-zinc-950 dark:text-white"
          }
          secondaryClassName={photoSrc ? "text-white/75" : "text-zinc-500"}
        />
      </div>
      <p
        className={cn(
          "mt-3 text-2xl font-extrabold tabular-nums",
          photoSrc ? "text-white drop-shadow-sm" : "text-zinc-950 dark:text-white"
        )}
      >
        {player.punkty.toFixed(2)}
        <span className={cn("ml-1 text-sm font-semibold", photoSrc ? "text-white/75" : "text-zinc-500")}>
          pkt
        </span>
      </p>
      <p className={cn("mt-1 text-xs", photoSrc ? "text-white/80" : "text-zinc-500")}>
