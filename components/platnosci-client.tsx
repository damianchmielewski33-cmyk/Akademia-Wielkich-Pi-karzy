...
function WalletBalanceHistory({
  ...
  if (loading && transactions.length === 0) {
    return (
      <p className="mt-4 flex items-center gap-2 rounded-xl border border-emerald-900/10 bg-emerald-50/20 px-4 py-8 text-sm text-zinc-600 dark:border-emerald-100/10 dark:bg-emerald-950/20 dark:text-zinc-400">
  ...
  if (transactions.length === 0) {
    return (
      <p className="mt-4 rounded-xl border border-dashed border-emerald-900/10 bg-emerald-50/20 px-4 py-8 text-center text-sm text-zinc-600 dark:border-emerald-100/10 dark:bg-emerald-950/20 dark:text-zinc-400">
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-xl border border-emerald-900/10 bg-white shadow-sm dark:border-emerald-100/10 dark:bg-zinc-900/90">
      <div className="flex items-center justify-between gap-2 border-b border-emerald-900/10 bg-emerald-50/50 px-4 py-2.5 dark:border-emerald-100/10 dark:bg-emerald-950/40">
        <p className="text-xs font-semibold uppercase tracking-wide text-emerald-900 dark:text-emerald-200">
          Ostatnie operacje
        </p>
        <span className="rounded-full bg-white/80 px-2 py-0.5 text-[11px] font-medium tabular-nums text-zinc-600 ring-1 ring-emerald-900/10 dark:bg-zinc-900/80 dark:text-zinc-300 dark:ring-emerald-100/10">
          {transactions.length}
        </span>
      </div>

      <div
        className="hidden grid-cols-[minmax(0,1.4fr)_5.5rem_5.5rem_5.5rem] gap-3 border-b border-emerald-900/10 bg-zinc-50/80 px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-zinc-500 dark:border-emerald-100/10 dark:bg-zinc-950/50 dark:text-zinc-400 sm:grid"
        aria-hidden
      >
        <span>Operacja</span>
      ...
      <ul className="max-h-[52vh] divide-y divide-emerald-900/8 overflow-y-auto dark:divide-emerald-100/8">
        {transactions.map((tx) => {
          ...
          return (
            <li
              key={tx.id}
              className={cn("border-l-4 bg-white px-4 py-3 dark:bg-zinc-900/50", meta.borderClass)}
            >
      ...
      {!isLoggedIn ? (
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Aktualne saldo i historia operacji na koncie.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div>
              <h3 className="text-sm font-semibold text-emerald-950 dark:text-emerald-100">Historia salda</h3>
