...
export default function ONasPage() {
  ...
  return (
    <div className="container mx-auto max-w-2xl flex-1 px-4 py-8 sm:py-10">
      <h1 className="text-3xl font-bold tracking-tight text-emerald-950">O akademii</h1>
      <p className="mt-2 text-sm text-zinc-600">
        Krótki przewodnik po tym, jak działa strona <strong className="font-semibold text-zinc-800">{SITE_NAME}</strong>.
      </p>

      <section className="mt-8 space-y-4 text-sm leading-relaxed text-zinc-700">
        <h2 className="text-lg font-semibold text-emerald-950">Zapisy na mecz</h2>
        <p>
          Terminy znajdziesz w zakładce <Link className="font-medium text-emerald-800 underline" href="/terminarz">Terminarz</Link>.
        ...
        <h2 className="pt-4 text-lg font-semibold text-emerald-950">Składy</h2>
        <p>
          Ustawienia drużyn na boisku publikuje administrator. Po udostępnieniu zobaczysz je w{" "}
          <Link className="font-medium text-emerald-800 underline" href="/sklady">
            Składach
          </Link>
          — dostępne są też wcześniejsze mecze, jeśli składy były publiczne.
        </p>

        <h2 className="pt-4 text-lg font-semibold text-emerald-950">Statystyki i rankingi</h2>
        <p>
          Po rozegranych meczach możesz uzupełnić swoje liczby (gole, asysty, dystans, obrony). Zbierają się one w{" "}
          <Link className="font-medium text-emerald-800 underline" href="/statystyki">
            Statystykach
          </Link>{" "}
          i{" "}
          <Link className="font-medium text-emerald-800 underline" href="/rankingi">
            Rankingach
          </Link>{" "}
          (wymagane konto).
        </p>
