  ...
  const users = (await db
    .prepare(
      `SELECT id, first_name, last_name, player_alias AS zawodnik
       FROM users
       WHERE (first_name LIKE ? OR last_name LIKE ? OR player_alias LIKE ?
    ...
    .prepare(
      Number.isFinite(matchId) && String(matchId) === q
        ? `SELECT id, match_date AS date, match_time AS time, location
           FROM matches WHERE id = ? LIMIT 8`
        : `SELECT id, match_date AS date, match_time AS time, location
           FROM matches
           WHERE (location LIKE ? OR match_date LIKE ? OR CAST(id AS TEXT) LIKE ?)
