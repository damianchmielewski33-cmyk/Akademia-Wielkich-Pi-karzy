...
/** Zatwierdza proponowany PIN (pin_hash_pending → pin_hash). */
export async function POST(_req: Request, ctx: Ctx) {
  ...
  await db
    .prepare(
      `UPDATE users SET
         ...
         pin_hash_pending = NULL,
         pin_reset_requested = 0,
  ...
  await logActivity(
    gate.session.userId,
    `Zatwierdził zmianę PIN-u użytkownika id ${userId}: ${row.first_name} ${row.last_name} (${row.player_alias})`
  );
