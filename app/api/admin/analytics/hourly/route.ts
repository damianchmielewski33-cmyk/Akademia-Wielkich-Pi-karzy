...
export async function GET() {
  ...
  const dayMeta = days.map((ymd) => {
    ...
    const anchor = Number.isNaN(startMs) ? null : new Date(startMs + 12 * 3600 * 1000);
    return {
      ymd,
  ...
  for (const { created_at } of rows) {
    const { ymd, hour } = wallClockYmdHourFromUtcIso(created_at, timeZone);
  ...
  return NextResponse.json({
    ...
    timeline_hourly,
    by_day,
    peak: { hour: peakHour, label: `${String(peakHour).padStart(2, "0")}:00`, views: peakViews },
  });
}
